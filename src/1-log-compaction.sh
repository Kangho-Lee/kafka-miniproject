...
kafka-topics.sh --zookeeper localhost:2181 --create --topic employee-salary --partitions 1 -replica-factor 1 --config cleanup.policy=compact --config min.cleanable.dirty.ratio=0.001 --config segment.ms=5000
...
kafka-console-producer.sh --broker-list localhost:9092 --topic employee-salary --property parse.key=true --property key.separator=,
